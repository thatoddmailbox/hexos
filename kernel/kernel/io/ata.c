// hey look yet another basekernel file!
// https://github.com/dthain/basekernel/blob/6fff9df12906787b16ba94d685c3ec5bf28eb1eb/src/ata.c
	...
	*/
	...
	if (ata_identify(id,ATA_COMMAND_IDENTIFY,cbuffer)) {
		...
		*blocksize = 2048;
		...
		printf("ata unit %d: identify command failed\n",id);
		return 0;
	}
...
void ata_init() {
	...
	irq_install_handler(ATA_IRQ0,ata_interrupt);
	irq_enable(ATA_IRQ0);

	irq_install_handler(ATA_IRQ1,ata_interrupt);
	irq_enable(ATA_IRQ1);
